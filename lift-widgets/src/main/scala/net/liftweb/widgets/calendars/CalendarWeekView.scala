...
class CalendarWeekView(val when: Calendar, val meta: WeekViewMeta) {
  ...
  private lazy val dateFormatter = new SimpleDateFormat("M/dd", meta.locale)
  private lazy val timeFormatter = new SimpleDateFormat("h a", meta.locale)
  ...
  def render(calendars: List[CalendarItem], itemClick: Can[AnonFunc]): NodeSeq = {
          ...
          (for (val day <- 0 to 6) yield 
              <td class="wkHeadCell">{
                try{
                  ...
                  weekDaysFormatter.format(time) + " " + dateFormatter.format(time)
                } finally {
                  headCal add(DAY_OF_MONTH, 1)
                }
              }</td>
    ...
    <div class="weekViewBody">
      <table cellspacing="0" cellpading="0" style="width: 100%;">
      {
        ...
        try{
          <tr>
            <td class="wkHour"><div>{Unparsed(timeFormatter format(cal getTime))}</div></td>
           {
