...
case class JSONArray[T <% JSONAny](elems: List[T]) extends JSONAny {
...
object JSONParser extends Parsers with ImplicitConversions { // with CombParserHelpers {
  ...
  def jsonObject: Parser[JSONObject] = ( spaces ~ '{' ~ spaces ~ members ~ spaces ~ '}' ~ spaces ^^ {case xs => JSONObject(xs)} )  |
  ...
  def elements = rep1sep(theValue, theValue, spaces ~ ',' ~ spaces)
