  ...
  def render(styles: NodeSeq): NodeSeq = {
		     ...
		     (styles \\ "notice_msg" \\ "@class"), 2)).flatMap
    {
      case (msg, titleList, defaultTitle, styleList, ord) =>
        ...
        if (!styles.isEmpty) {
          ord match {
            case 0 => MsgsErrorMeta(Full(AjaxMessageMeta(Full(title), Full(styles.mkString(" ")))))
            case 1 => MsgsWarningMeta(Full(AjaxMessageMeta(Full(title), Full(styles.mkString(" ")))))
            case 2 => MsgsNoticeMeta(Full(AjaxMessageMeta(Full(title), Full(styles.mkString(" ")))))
         }
        }
