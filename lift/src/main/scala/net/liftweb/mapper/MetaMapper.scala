...
trait MetaMapper[A<:Mapper[A]] extends BaseMetaMapper with Mapper[A] {self: A =>
  ...
  def save(toSave: A): Boolean = {
	...
	_beforeCreate(toSave)
        ...
        def prepStat(st : PreparedStatement) : Boolean = {
	    var colNum = 1
	  ...
	  for (col <- mappedColumns) {
	    if (!columnPrimaryKey_?(col._1)) {
          ...
          val oneRowUpdated : Boolean = (conn.brokenAutogeneratedKeys_?, indexMap) match {
            case (true, null)  => hasOneRow(st.executeQuery)
            case (true, _)     => runAppliers(st.executeQuery)
            case (false, null) => st.executeUpdate == 1
            case (false, _)    => {
        ...
        val ret = if (conn.brokenAutogeneratedKeys_?) {
