...
trait FileDbSetup {
  ...
  def setupDB {
    ...
    def logger(s: => AnyRef) = Log.info(s) 
    Schemifier.schemify(true, logger _, User, Pet)
    Schemifier.schemify(true, logger _, TestStateMachine)
  }
}
...
trait DbSetup {
  ...
  def setupDB {
    ...
    def logger(s: => AnyRef) = Log.info(s) 
    Schemifier.schemify(true, logger _, User, Pet, TestStateMachine)
  }
}
